"""Command line interface for clingen-ingest."""
import logging
...
def callback(version: bool = typer.Option(False, "--version", is_eager=True),
    ...
    """clingen-ingest CLI."""
    if version:
        from clingen_ingest import __version__
        typer.echo(f"clingen-ingest version: {__version__}")
        raise typer.Exit()


@app.command()
def download(force: bool = typer.Option(False, help="Force download of data, even if it exists")):
    """Download data for clingen-ingest."""
    typer.echo("Downloading data for clingen-ingest...")
...
def transform(
    ...
    """Run the Koza transform for clingen-ingest."""
    typer.echo("Transforming data for clingen-ingest...")
